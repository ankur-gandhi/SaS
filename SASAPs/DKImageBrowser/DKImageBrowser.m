...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        self.currentPage = 0;
        self.view.backgroundColor = [UIColor redColor]; //self.DKBackgroundColor;
    }
    return self;
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    switch (_DKThumbnailStripPosition) {
        ...
        case DKThumbnailStripPositionBottom: {
                
                CGRect frame = _DKThumbnailCollectionView.frame;
                frame.origin.y = self.view.bounds.size.height - 120;
                _DKThumbnailCollectionView.frame = frame;
                
                frame = _DKImageCollectionView.frame;
                frame.origin.y = self.view.bounds.origin.y + 25;
                _DKImageCollectionView.frame = frame;
            }
            break;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)cview cellForItemAtIndexPath:(NSIndexPath *)indexPath {    
    if ((cview==_DKImageCollectionView) &&
        (indexPath.item>1)) {
        [_DKThumbnailCollectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:indexPath.item inSection:0] atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:YES];
    }
